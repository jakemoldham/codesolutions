class PostController < Amber::Controller::Base
  ...
  def create
  post = Post.new(params.to_h.select(["title", "author", "body", "spotlight", "description"]))
  ...
  def update
    if post = Post.find(params["id"])
      post.set_attributes(params.to_h.select(["title", "author", "body", "spotlight", "description"])) 
      if post.valid? && post.save
